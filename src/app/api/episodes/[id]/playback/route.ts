...
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (!listed.Contents || listed.Contents.length === 0) {
      return NextResponse.json(
        { error: "Nenhum arquivo encontrado no prefixo HLS deste episódio." },
        { status: 404 },
      );
    }
    ...
    if (hlsObject && hlsObject.Key) {
      ...
      return NextResponse.json(
        {
          error:
            "Nenhum arquivo HLS (.m3u8) ou vídeo compatível (mp4/m4v/mov/mkv/webm/avi) encontrado no prefixo HLS deste episódio.",
        },
        { status: 400 },
      );
    }
